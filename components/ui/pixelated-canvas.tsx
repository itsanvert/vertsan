  ...
  React.useEffect(() => {
    ...
    const compute = () => {
      ...
      if (tintColor && tintStrength > 0) {
        ...
        tintRGB = parse(tintColor) as [number, number, number];
      }
    ...
    img.onload = () => {
      ...
      (img as HTMLImageElementWithCleanup)._cleanup = cleanup;
    };
    ...
    if (responsive) {
      ...
      return () => {
        ...
        if ((img as HTMLImageElementWithCleanup)._cleanup)
          (img as HTMLImageElementWithCleanup)._cleanup();
      };
    }
...
interface HTMLImageElementWithCleanup extends HTMLImageElement {
  _cleanup: () => void;
}
